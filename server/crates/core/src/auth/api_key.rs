...
pub async fn authenticate(db: &PgPool, api_token: &str) -> Result<User, AppError> {
    let hash = token::hash_api_token(api_token); // SHA256 since its fast, argon is probably better but slower
    ...
    if let Err(e) = sqlx::query("UPDATE api_keys SET last_used_at = now() WHERE token_hash = $1")
        .bind(&hash)
        .execute(db)
        .await
    {
...
impl<S: HasDb + 'static> FromRequest for ApiKeyUser<S> {
    ...
    fn from_request(req: &HttpRequest, payload: &mut Payload) -> Self::Future {
        ...
        Box::pin(async move {
            ...
            let user = authenticate(state.db(), bearer.token()).await?;
